 ...
 * - Input: an array of float (3 rotation angles)
...
public:
    ...
    Pano2Perspective(int w=480, int h=360, float fov=90.0);
    // Initialize using panorama image size and fov (x,y in deg)
    Pano2Perspective(int pano_w, int pano_h, float fov_x, float fov_y);
    ~Pano2Perspective();

    // Getter
    cv::Mat process_image(array<Eigen::Matrix<unsigned char, Dynamic, Dynamic, RowMajor>, 3> &src);
    array<float, 4> get_intrinsics();

    // Setter
    void init_values(int w, int h, float fov);
    void set_rotation(array<float, 3> &angles);
