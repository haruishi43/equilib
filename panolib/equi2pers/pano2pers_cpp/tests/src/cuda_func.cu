...
{    
    ...
    SAFE_CALL(cudaMallocManaged<unsigned char>(&d_pano, panoBytes),"CUDA Malloc Failed");
    SAFE_CALL(cudaMallocManaged<unsigned char>(&d_pers, persBytes),"CUDA Malloc Failed");
    SAFE_CALL(cudaMallocManaged<double>(&d_im2ori, 9*sizeof(double)),"CUDA Malloc Failed");
